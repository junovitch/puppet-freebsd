define freebsd::network::interface (
  ...
  # for a given interface.  You may also wish to only set the MTU and bring up
  # the interface, which can be useful when you want ot assign a given
  # interface to a lagg, but also want a custom MTU.

  # We also provide an option here for $alias.  This is used to add virtual addresses to a physicial interface.

  Shell_config { file => '/etc/rc.conf' }

  # The string to use in the ifconfig line for rc.conf.
  if ( $mtu != '' ) {
    $mtu_string = " mtu ${mtu}"
  }
